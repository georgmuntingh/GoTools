...
int main( int argc, char* argv[] )
{
  ...
  double coefs1[24] = {0.0, 0.0, 0.0, 3.0, 0.0, 0.0,
  ...
  double kink = 0.01;
  double bend = 0.05;
  shared_ptr<ParamVolume> pvol1 = vol1;
  shared_ptr<ftVolume> topvol1(new ftVolume(pvol1, gap, kink));
  shared_ptr<ftSurface> face(new ftSurface(sf1, 0));

  // Split volume
  double eps = 1.0e-4;
  vector<shared_ptr<ftVolume> > splitvol = 
    ftVolumeTools::splitVolumes(topvol1, face, eps);
  
  double coefs2[24] = {3.0, 0.0, 0.0, 6.0, 0.0, 0.0,
		       3.0, 3.0, 0.0, 6.0, 3.0, 0.0,
		       3.0, 0.0, 3.0, 6.0, 0.0, 3.0,
		       3.0, 3.0, 3.0, 6.0, 3.0, 3.0};
  shared_ptr<SplineVolume> vol2(new SplineVolume(2, 2, 2, 2, 2, 2,
						 &knots1[0], &knots2[0],
						 &knots3[0], &coefs2[0],
						 3));

  // Degree elevation
  vol2->raiseOrder(2, 2, 2);

  // Knot insertion in all parameter direction
  vol2->insertKnot(0, internal_knots);
  vol2->insertKnot(1, internal_knots);
  vol2->insertKnot(2, internal_knots);

  shared_ptr<ParamVolume> pvol2 = vol2;
  shared_ptr<ftVolume> topvol2(new ftVolume(pvol2, gap, kink));

  vector<shared_ptr<ftVolume> > volumes;
  // Disconnect the first trimmed volume from the second
  splitvol[0]->eraseBodyAdjacency();

  // Specify volumes for isogeometric model
  volumes.push_back(splitvol[0]);
  volumes.push_back(topvol2);
...
//  vector<pair<ParamSurface*, Point> > bd_pts;
  ...
  vector<double> knt1 = sol->knots(0);
  cout << "Sol, knots in dir 0: ";
  for (ki=0; ki<(int)knt1.size(); ++ki)
    {
      if (ki > 0)
	cout << ", ";
      cout << knt1[ki];
  ...
  vector<double> knt2 = sol->distinctKnots(0);
  cout << "Sol, distinct knots in dir 0: ";
  for (ki=0; ki<(int)knt2.size(); ++ki)
    {
      if (ki > 0)
	cout << ", ";
      cout << knt2[ki];
  ...
  for (ki=1; ki<(int)knt2.size(); ++ki)
    newknots.push_back(0.5*(knt2[ki-1]+knt2[ki]));
